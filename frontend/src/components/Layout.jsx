...
export const Layout = () => {
  ...
  return (
    <div className="min-h-screen text-[var(--color-text)] font-sans relative overflow-hidden">
      ...
      {/* Navigation */}
      <nav className="sticky top-0 z-40 bg-[var(--color-accent)] border-b-4 border-[var(--color-border)] shadow-[0_4px_0_0_var(--color-shadow)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-24">
            ...
            <div className="flex items-center gap-4">
              {isAuthenticated ? (
                <>
                  <Link to="/report" className="hidden sm:flex items-center gap-2 px-5 py-3 comic-button bg-[var(--color-secondary)] text-[var(--color-text)]">
                    <Plus size={24} strokeWidth={4} />
                    <span className="text-lg">Report Item</span>
                  </Link>
                  <div className="flex items-center gap-3 pl-4 border-l-4 border-[var(--color-border)] h-12 ml-2">
                    <div className="w-12 h-12 rounded-full bg-[var(--color-primary)] border-4 border-[var(--color-border)] flex items-center justify-center text-xl font-black text-white shadow-[3px_3px_0_0_var(--color-shadow)] rotate-3">
                      {user?.email?.[0] || 'U'}
                    </div>
                    <button 
                      onClick={logout}
                      className="p-2 text-[var(--color-text)] hover:bg-black hover:text-white rounded-xl transition-colors border-2 border-transparent hover:border-[var(--color-border)]"
                      title="Log out"
                    >
                ...
                <button
                  ...
                  className="px-8 py-3 comic-button text-xl bg-[var(--color-surface)] text-[var(--color-text)] -rotate-2 hover:rotate-0"
                >
                  Sign In
                </button>
              )}
